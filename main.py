...
#Base de datos Historica, para overview de la empresa
...
#Asignacion de calificacion de sucursales, en base al porcentaje de entregas tardias y promedio de dias de entrega.
...
#codigo para no mostrar el menu de streamlit cuando no esta deployed.
...
#CSS inicial, cubre elementos globales como cards y otros aspectos generales.
...
#css mas especifico para la tabla de sucursales en el primer tab
...
#Inicializacion de los 4 tabs que va tener nuestro dashboard:
...
######
with tab1:
    #iniciamos con la tabla de estados/sucusales
    ...
    #Tabla de Sucursales focused:
        ...
        #visualizacion entregas por dia bar graph
        with col2:
            if st.session_state.selected_sucursal:
                df_charts = df_filtered[df_filtered['sucursal_asignada'] == st.session_state.selected_sucursal]
            else:
    ...
    if st.session_state.tabla_activa == 'estados':
        ...
        #visualizacion de top categorias 
...
def aplicar_modelo_ml(df_input, _model, _preprocessor, features):
    try:
        ...
        #funcion principal para el forecast
...
##############################################################################################################################################
####################################################  FIN DEL MODELO DE MACHINE LEARNING  ####################################################
##############################################################################################################################################
    ...
    if df_usuario is not None and model is not None:
        ...
        with col_cards:
            if 'sucursal_asignada' in df_prediccion.columns and 'estado_cliente' in df_prediccion.columns:
                ...
                ###
                #CSS para mejorar la visualizacion de las alarmas del tab 2 forecast#
                ###
                cards_html = """
                <style>
                ...
                for _, row in sucursales_stats_sorted.iterrows():
                    ...
                    #intento 2 de acortar nombre para mejor visualizacion
        ...
        #preparacion de columnas para mostrar 
        ...
        #columnas selecionadas y renombre.
        nuevos_nombres = {}
        ...
        st.error("Error en el entrenamiento del modelo.")
    else:
        st.warning("Erro, problema inesperado.")
######
#TAB2
######
...
with tab3:
    ...
    #columna de tabla de informacion de dias de retraso por estado
    with col_table:
        st.markdown("""
    ...
    #columna de cantidades en pesos
    with col_results:
